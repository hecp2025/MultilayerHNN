...
def main(params):
    ...
    if train_eval=='train': 
        ...
        for indice in tqdm(range(len(num_pattern_list))):
            ...
            if params['data_type']=='binary':
                if num_pattern > 256:
                    ...
                    learn_iters = params['learn_iters'] * 4
                    learn_lr = params['learn_lr'] / 2
                elif num_pattern > 16:
                    learn_iters = params['learn_iters'] * 2
                    learn_lr = params['learn_lr'] 
            else:
                if num_pattern > 256:
                    learn_iters = params['learn_iters'] * 4
        ...
        # 保存数据
        if params['data_type']=='binary':
            with open(model_path + f'/Multilayer_capacity_{params['dataset']}_{num_dimen}_hd={hidden_dim}_stuck={stuck_at_fault_rate}_simulation.pkl', 'wb') as f:
                pickle.dump({
            ...
            with open(model_path + f'/Multilayer_capacity_{params['dataset']}_continuous_{num_dimen}_hd={hidden_dim}_stuck={stuck_at_fault_rate}_simulation.pkl', 'wb') as f:
                pickle.dump({
                    'num_pattern_list': num_pattern_list,
                    'stored_patterns': stored_patterns,
                    'MultiHNN_weights': MultiHNN_weights
        ...
        if params['data_type']=='binary':
            with open(model_path + f'/Multilayer_capacity_{params['dataset']}_{num_dimen}_hd={hidden_dim}_stuck={stuck_at_fault_rate}_simulation.pkl', 'rb') as f:
            # with open(model_path + f'AssociateMemory_capacity_random_256_simulation.pkl', 'rb') as f:
                data = pickle.load(f)
        else:
            with open(model_path + f'/Multilayer_capacity_{params['dataset']}_continuous_{num_dimen}_hd={hidden_dim}_stuck={stuck_at_fault_rate}_simulation.pkl', 'rb') as f:
                data = pickle.load(f)
        ...
        for indice in tqdm(range(len(num_pattern_list))):
            ...
            Encoder_weight=MultiHNN_weights['encoder'][indice]
            ...
            with torch.no_grad():
                ...
                for i,layer in enumerate(multi_hnn.encoder):
                    if i%2==0:
                        multi_hnn.encoder[i].weight=torch.nn.Parameter(torch.from_numpy(Encoder_weight).float())
                multi_hnn.decoder[0].weight=torch.nn.Parameter(torch.from_numpy(Decoder_weight).float())
        ...
        # 保存数据
        if params['data_type']=='binary':
            np.savez(num_path +f'/Multilayer_capacity_{params['dataset']}_{num_dimen}_hd={hidden_dim}_stuck={stuck_at_fault_rate}_var={mem_var}_corr={corruption_rate}_simulation.npz',
            ...
            np.savez(num_path +f'/Multilayer_capacity_{params['dataset']}_continuous_{num_dimen}_hd={hidden_dim}_stuck={stuck_at_fault_rate}_var={mem_var}_corr={corruption_rate}_simulation.npz',
                 num_pattern_list=num_pattern_list,MultiHNN_similarity_list=MultiHNN_similarity_list,input_similarities=input_similarities)      
